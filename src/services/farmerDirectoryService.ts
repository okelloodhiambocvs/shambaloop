...
export async function fetchFarmerDirectory(token: string, signal?: AbortSignal): Promise<InvestorFarmerProfile[]> {
  const response = await fetch('/api/investor/farmers', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store', signal });
