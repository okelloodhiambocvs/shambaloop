...
import { 
  Tractor, Sparkles, Award, Wallet, ShieldCheck, Plus, Filter, 
  MapPin, RefreshCw, BarChart2, PlusCircle, CheckCircle, FileText,
  UserCheck, AlertTriangle, MessageSquare, ChevronDown, Check, Coins, Layers,
  Sun, Moon, LogOut, LogIn, UserPlus, X, Search
} from 'lucide-react';
...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    ...
    if (stored) {
      try {
        ...
        if (matchingSeed) {
          localStorage.setItem('sl_current_user', JSON.stringify(matchingSeed));
          return matchingSeed;
        }
        return u;
      } catch (err) {
    ...
    const defaultUser = CANONICAL_SEED_USERS[UserRole.LANDOWNER];
    localStorage.setItem('sl_current_user', JSON.stringify(defaultUser));
    return defaultUser; // Start with Wanjiku Kamau (Landowner) logged in by default
  });
  ...
  useEffect(() => {
    ...
    const approvedSeedIds = ['user_1', 'user_2', 'user_3', 'user_admin'];
  ...
  }, [listings, leases, partnerships, verifications, usersList]);
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen flex flex-col font-sans transition-all duration-300 bg-bg-base text-text-base border-border-base" id="auth_gate_wrapper">


        <div className="flex-1 flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-md w-full space-y-8 p-8 rounded-3xl border shadow-xl bg-card-bg border-border-base relative overflow-hidden transition-all duration-300" id="auth_card">
          {/* Subtle background decoration */}
          <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-emerald-500/10 blur-2xl"></div>
          <div className="absolute -bottom-10 -left-10 w-32 h-32 rounded-full bg-amber-500/10 blur-2xl"></div>

          {/* Core branding */}
          <div className="flex flex-col items-center text-center">
            <Logo size={56} variant="symbol" className="justify-center select-none" />
            <h2 className="mt-3.5 text-2xl font-black font-display tracking-tight leading-none text-[#1F6B3D] dark:text-[#2e8b54]">
              Shamba<span className="text-[#8B5E3C] dark:text-[#a37350]">Loop</span>
            </h2>
            <p className="mt-1 text-[11px] text-[#8B5E3C] dark:text-slate-400 font-bold tracking-wide uppercase">
          ...
          <div className="flex gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-6">
            <button
              ...
              className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${authTab === 'login' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white'}`}
            ...
            <button
              ...
              className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${authTab === 'register' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white'}`}
              id="auth_tab_register_btn"
            >
              Create Account
            </button>
          </div>
          ...
          {authError && (
            <div className="p-3.5 text-xs bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-900 rounded-xl flex items-center gap-2" id="auth_error_alert">
              <span>{authError}</span>
            </div>
          )}

          {authSuccessMsg && (
            <div className="p-3.5 text-xs bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 rounded-xl flex items-center gap-2" id="auth_success_alert">
          ...
          {authTab === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4" id="login_form">
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">Phone Number</label>
                <input
                  type="text"
                  required
              ...
              <button
                type="submit"
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all cursor-pointer shadow-md"
                id="login_submit_btn"
              >
                Enter Ecosystem Dashboard
              </button>
            </form>
          ) : (
            <form onSubmit={handleSignUp} className="space-y-4" id="register_form">
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">Full Name</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Josphat Kiprop"
                  value={authName}
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">Email (Optional)</label>
                <input
                  type="email"
                  placeholder="e.g. kiprop@gmail.com"
                  value={authEmail}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">County</label>
                  <select
                    value={authCounty}
                    onChange={(e) => setAuthCounty(e.target.value)}
                    className="w-full p-3 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    id="register_county_select"
                  >
                    {COUNTIES_LIST.filter(c => c !== 'All Counties').map(c => (
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">Ecosystem Role</label>
                  <select
                    value={authRole}
          ...
          <div className="flex justify-between items-center pt-2.5" id="auth_footer">
            <span className="text-[10px] text-slate-400 select-none">ShambaLoop Sandbox</span>
            
            {/* Theme Toggle Button */}
            <button
              onClick={toggleTheme}
              className="p-1.5 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition cursor-pointer text-[10px] uppercase font-bold tracking-wider"
              title="Toggle Theme"
              id="auth_theme_toggle_btn"
              type="button"
            >
  ...
  return (
    <div className="min-h-screen pb-16 flex flex-col font-sans transition-all duration-300 bg-bg-base text-text-base border-border-base" id="shambaloop_app_stage">
            ...
            <div className={`flex items-center gap-2 p-1.5 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
              ...
              <div className="relative flex items-center">
                <select
                  ...
                  onChange={(e) => {
                    const u = usersList.find(u => u.id === e.target.value);
                    if (u) {
                      setCurrentUser(u);
                      localStorage.setItem('sl_current_user', JSON.stringify(u));
                      setActivePartnershipForYieldLog('');
                      setActivePartnershipForHealthLog('');
                    }
                  }}
                  className={`pl-2.5 pr-8 py-1.5 rounded-lg text-xs font-bold focus:outline-none appearance-none transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-800'} cursor-pointer`}
                  id="header_user_switcher"
                >
                  {usersList.map((u) => (
                    <option key={u.id} value={u.id}>{u.name} ({u.role.toUpperCase()})</option>
                  ))}
                </select>
                <div className={`absolute inset-y-0 right-0 flex items-center pr-2.5 pointer-events-none ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                  <span className="text-[9px] font-bold select-none">v</span>
                </div>
              </div>
              ...
              <button
                ...
                className={`p-1.5 px-2.5 rounded-lg hover:text-rose-500 transition-colors cursor-pointer ${isDarkMode ? 'text-slate-400 hover:bg-slate-700/50' : 'text-slate-600 hover:bg-slate-200'}`}
                title="Logout from Core"
                id="header_logout_btn"
              >
                <span className="text-[10px] font-bold uppercase tracking-wider font-sans">Exit</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Floating Sticky helper footer for instant contact simulation */}
      <footer className="mt-16 bg-[#1a1a1a] text-slate-300 select-none border-t border-slate-800 text-xs font-sans" id="shambaloop_custom_footer">
        {/* Newsletter Section */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2 text-slate-400 font-medium font-sans">
                ...
                <li><button onClick={() => openFooterDoc('FAQ')} className="text-emerald-400 hover:text-emerald-305 font-bold transition hover:underline text-left cursor-pointer">Frequently Asked Questions (FAQ)</button></li>
            ...
            <div className="space-y-3">
              <h4 className="text-white font-bold text-xs uppercase tracking-wider">USEFUL LINKS</h4>
              <ul className="space-y-1.5 text-slate-400 font-medium text-left font-sans">
                <li>
                  <button 
                    onClick={() => openFooterDoc('FAQ')} 
                    className="text-emerald-400 font-bold hover:text-emerald-300 transition hover:underline text-left cursor-pointer flex items-center gap-1.5"
                ...
                <li><button onClick={() => openFooterDoc('Track Your Order')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Track Your Order</button></li>
                <li><button onClick={() => openFooterDoc('Shipping and delivery')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Shipping and delivery</button></li>
                <li><button onClick={() => openFooterDoc('Pick-up Stations')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Pick-up Stations</button></li>
                <li><button onClick={() => openFooterDoc('Return Policy')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Return Policy</button></li>
                <li><button onClick={() => openFooterDoc('How to Order?')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">How to Order?</button></li>
                <li><button onClick={() => openFooterDoc('Dispute Resolution Policy')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Dispute Resolution Policy</button></li>
                <li><button onClick={() => openFooterDoc('Corporate and Bulk Purchase')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Corporate and Bulk Purchase</button></li>
                <li><button onClick={() => openFooterDoc('Advertise with ShambaLoop')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Advertise with ShambaLoop</button></li>
                <li><button onClick={() => openFooterDoc('Report a Product')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Report a Product</button></li>
                <li><button onClick={() => openFooterDoc('ShambaLoop Payment Information Guidelines')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer text-xs leading-none">ShambaLoop Payment Guidelines</button></li>
                <li><button onClick={() => openFooterDoc('Black Friday')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Black Friday</button></li>
              </ul>
            </div>

            {/* COLUMN 3: ABOUT SHAMBALOOP */}
            <div className="space-y-3">
              ...
              <ul className="space-y-1.5 text-slate-400 font-medium text-left">
                <li><button onClick={() => openFooterDoc('About Us')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">About us</button></li>
                <li><button onClick={() => openFooterDoc('Returns and Refunds Policy')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Returns and Refunds Policy</button></li>
                <li><button onClick={() => openFooterDoc('ShambaLoop Careers')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">ShambaLoop Careers</button></li>
                <li><button onClick={() => openFooterDoc('ShambaLoop Express')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">ShambaLoop Express</button></li>
                <li><button onClick={() => openFooterDoc('Terms and Conditions')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Terms and Conditions</button></li>
                <li><button onClick={() => openFooterDoc('Store Credit Terms and Conditions')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer text-xs">Store Credit Terms</button></li>
                <li><button onClick={() => openFooterDoc('Privacy Notice')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Privacy Notice</button></li>
                <li><button onClick={() => openFooterDoc('Cookies Notice')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Cookies Notice</button></li>
                <li><button onClick={() => openFooterDoc('Flash Sales')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Flash Sales</button></li>
              </ul>
            </div>

            {/* COLUMN 4: MAKE MONEY WITH SHAMBALOOP */}
            <div className="space-y-3">
              <h4 className="text-white font-bold text-xs uppercase tracking-wider">MAKE MONEY WITH SHAMBALOOP</h4>
              <ul className="space-y-1.5 text-slate-400 font-medium text-left">
                <li><button onClick={() => openFooterDoc('Sell on ShambaLoop')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Sell on ShambaLoop</button></li>
                <li><button onClick={() => openFooterDoc('Vendor Hub')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Vendor Hub</button></li>
                <li><button onClick={() => openFooterDoc('Become a Sales Consultant')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer">Become a Sales Consultant</button></li>
                <li><button onClick={() => openFooterDoc('Become A ShambaLoop Order Point')} className="hover:text-emerald-400 transition hover:underline text-left cursor-pointer text-xs">Become A ShambaLoop Order Point</button></li>
              </ul>
            </div>
