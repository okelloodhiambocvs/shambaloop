...
export default function FarmerOperations({ user, partnerships, mode }: Props) {
  ...
  if (mode === 'wallet') {
    return (
      <section className="space-y-6" id="farmer_wallet_view">
        <div>
          <h3 className="text-sm font-bold dark:text-white">Cooperative Farm Wallet</h3>
          <p className="text-xs text-slate-500">
            Immutable server-ledger balances and dedicated M-Pesa purpose disbursements.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-white dark:bg-slate-900 shadow-xs">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Available Earnings</span>
            ...
            <span className="text-[10px] text-slate-400">Ready for mobile disbursement</span>
          </div>
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-white dark:bg-slate-900 shadow-xs">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Farm Operating Escrow</span>
            <strong className="mt-1 block text-xl text-amber-700 dark:text-amber-400 font-extrabold">
              KES {(wallet?.operationalFarmFundsKES || 0).toLocaleString()}
            </strong>
            <span className="text-[10px] text-slate-400">Allocated for inputs & care</span>
          </div>
        </div>

        {/* Deposit with Purpose Dropdown */}
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-slate-50/60 dark:bg-slate-900/40">
          <strong className="block text-xs font-bold dark:text-white mb-2 flex items-center gap-2">
            <Wallet className="h-4 w-4 text-emerald-600" />
            Initiate Wallet Deposit / Payment Intent
          </strong>
          <form
            ...
            onSubmit={e => {
              ...
              const purpose = String(f.get('purpose') || depositPurpose);
              void action(async () => {
                const r = await farmerWorkspaceApi.createDepositIntent(amount, crypto.randomUUID(), purpose);
                if (r.error) throw new Error(r.error);
                e.currentTarget.reset();
              }, `Payment intent initiated for ${purpose}.`);
            }}
          >
            <label className="block">
              <span className="text-[11px] font-bold text-slate-600 dark:text-slate-300">Transaction Purpose</span>
              <select
                name="purpose"
                ...
                id="select_deposit_purpose"
              >
                <option value="FEED_PURCHASE">Livestock Feed & Nutrition (Hay/Silage)</option>
                <option value="VETERINARY_FEES">Veterinary Care & Diagnostic Fees</option>
                <option value="EQUIPMENT_LEASE">Equipment Lease & Field Tillage</option>
                <option value="OPERATIONAL_FUNDS">General Working Capital / Inputs</option>
                <option value="ESCROW_SECURITY">Cooperative Escrow Guarantee Lockup</option>
              </select>
            </label>
            <label className="block">
            ...
            <div className="flex items-end">
              <button
                disabled={busy}
                type="submit"
                className="action-primary w-full h-[38px] cursor-pointer"
                id="btn_create_payment_intent"
              >
                {busy ? 'Processing...' : 'Create Payment Intent'}
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-2 dark:border-slate-800">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                id="filter_transactions_by_purpose"
              >
                <option value="ALL">All Purpose Categories</option>
                <option value="FEED">Livestock Feed & Nutrition</option>
                <option value="VETERINARY">Veterinary Services</option>
                <option value="EQUIPMENT">Equipment & Tillage</option>
                <option value="OPERATIONAL">Operational / Earnings</option>
