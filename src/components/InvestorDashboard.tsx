...
import { BriefcaseBusiness, ClipboardList, FileText, MapPin, PlusCircle, ShieldAlert, Star, Stethoscope, UsersRound } from 'lucide-react';
...
import { InvestorBriefDocuments } from './investor/InvestorBriefDocuments';
...
export default function InvestorDashboard(props: Props) {
  ...
  const farmers = useMemo(() => visibleFarmerProfiles(props.farmers).filter(farmer => {
  ...
  return <section id="investor_primary_dashboard" className="space-y-5">
    <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 dark:border-slate-800 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Investor workspace</p>
        <h1 className="mt-1 text-2xl font-bold text-slate-950 dark:text-white">Hello, {props.currentUser.name}</h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Welcome to your cooperative investor workspace. Manage Projects, opportunities, and decisions that need your attention.</p>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <nav className="flex flex-wrap gap-2" aria-label="Investor workspace views">
          {([['overview', 'Overview'], ['farmers', 'Find farmers'], ['brief', 'Investment brief'], ['fms', 'Farm Management System'], ['wallet', 'Wallet'], ['disputes', 'Dispute Room'], ['reviews', 'Reviews']] as const).map(([id, label]) => <button key={id} type="button" onClick={() => { setView(id); if (id === 'farmers') void props.onRefreshFarmers?.(); }} className={`rounded-lg px-3 py-2 text-xs font-semibold ${view === id ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200'}`}>{label}</button>)}
        </nav>
      </div>
    </header>
            ...
            <article key={listing.id} className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 shadow-xs">
              <div>
                <div className="flex items-start justify-between gap-2">
                  <span className="rounded bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300">{listing.type}</span>
              ...
              {props.onListingAction && (
                <button type="button" onClick={() => props.onListingAction?.(listing)} className="mt-4 w-full rounded-lg bg-emerald-700 py-1.5 text-center text-xs font-semibold text-white hover:bg-emerald-800 transition cursor-pointer">
                  Evaluate & Connect
                </button>
              )}
            </article>
    ...
    {view === 'farmers' && <section className="space-y-4" aria-busy={props.farmersLoading}>
      <div className="flex items-center justify-between"><h2 className="font-semibold">Find farmers</h2><button type="button" disabled={props.farmersLoading} onClick={() => void props.onRefreshFarmers?.()} className="rounded border px-3 py-2 text-xs disabled:opacity-50">Refresh farmers</button></div>
      <div className="flex flex-col gap-3 sm:flex-row">
      ...
      {props.farmersLoading ? <p role="status">Loading farmers…</p> : props.farmersError ? <p role="alert" className="text-red-700">{props.farmersError}</p> : farmers.length ? <div className="grid gap-3 lg:grid-cols-2">{farmers.map(farmer => <article key={farmer.id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-bold text-slate-900 dark:text-white">{farmer.name}</h2><p className="text-xs text-slate-500">{farmer.county}{farmer.verified ? ' · Verified profile' : ''}</p></div><UsersRound className="h-5 w-5 text-emerald-600"/></div><p className="mt-3 text-xs text-slate-600 dark:text-slate-300">{farmer.farmSpecialties?.length ? farmer.farmSpecialties.join(' · ') : 'Farm details have not been added.'}</p><div className="mt-3 border-t border-slate-100 pt-3 text-xs dark:border-slate-800"><strong className="text-slate-800 dark:text-slate-100">Approved listings</strong>{farmer.listings.length ? <ul className="mt-1 space-y-1 text-slate-600 dark:text-slate-300">{farmer.listings.slice(0, 2).map(listing => <li key={listing.id}>{listing.title} · {money(listing.priceKES)}</li>)}</ul> : <p className="mt-1 text-slate-500">No approved listings.</p>}</div><p className="mt-3 text-xs text-slate-500">{proposals.some(proposal => proposal.farmerId === farmer.id) ? 'A proposal from this farmer is ready for review.' : 'No proposal has been sent to you.'}</p></article>)}</div> : <Empty text={county || sector ? "No farmers match these filters." : "No farmers have joined yet. Registered farmer profiles will appear here."} />}
    </section>}

    {view === 'brief' && <section className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <BriefcaseBusiness className="h-5 w-5 text-emerald-700"/>
        <h2 className="mt-3 text-sm font-bold text-slate-900 dark:text-white">Your investment opportunity</h2>
        <p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">Describe the capital or resources you provide and the partner you need. Farmers can discover these public requirements; private records are not exposed.</p>
        <div className="mt-5 space-y-3 text-xs"><p><strong>Capital:</strong> {brief.budgetKES ? money(brief.budgetKES) : 'Not set'}</p><p><strong>Focus:</strong> {brief.preferredSectors.join(', ') || 'Not set'}</p><p><strong>Counties:</strong> {brief.targetCounties || 'Not set'}</p></div>
        <InvestorBriefDocuments criteria={props.criteria} />
      </div>
      <form onSubmit={saveBrief} className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <Field label="Partnership type"><select value={brief.lookingFor} onChange={event => setBrief(current => ({ ...current, lookingFor: event.target.value as InvestorCriteria['lookingFor'] }))}>{models.map(model => <option key={model.value} value={model.value}>{model.label}</option>)}</select></Field>
        <Field label="Capital available (KES)"><input required min="1" max="100000000" type="number" value={brief.budgetKES || ''} onChange={event => setBrief(current => ({ ...current, budgetKES: Number(event.target.value) }))}/></Field>
        <Field label="What I provide"><textarea required maxLength={1000} value={brief.resourcesProvided} onChange={event => setBrief(current => ({ ...current, resourcesProvided: event.target.value }))} placeholder="Capital, equipment, market access, land, or other resources" /></Field>
        <Field label="What I need from a partner"><textarea required maxLength={1000} value={brief.partnerRequirements} onChange={event => setBrief(current => ({ ...current, partnerRequirements: event.target.value }))} placeholder="Experience, land, activity, management expertise, and partnership terms" /></Field>
        <div><p className="mb-2 text-xs font-semibold text-slate-700 dark:text-slate-200">Focus areas</p><div className="flex flex-wrap gap-2">{sectors.map(item => <button key={item} type="button" onClick={() => toggleSector(item)} className={`rounded-full px-3 py-1.5 text-xs ${brief.preferredSectors.includes(item) ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200'}`}>{item}</button>)}</div></div>
        <Field label="Target counties (separate with commas)"><input required value={brief.targetCounties} onChange={event => setBrief(current => ({ ...current, targetCounties: event.target.value }))}/></Field>
        <Field label="Additional notes"><textarea required maxLength={1500} value={brief.notes} onChange={event => setBrief(current => ({ ...current, notes: event.target.value }))} placeholder="Any other practical requirements" /></Field>
        <button disabled={busy === 'brief'} className="rounded-lg bg-emerald-700 hover:bg-emerald-800 px-4 py-2 text-xs font-semibold text-white transition disabled:opacity-60">{busy === 'brief' ? 'Saving...' : 'Save investment brief'}</button>
      </form>
    </section>}
