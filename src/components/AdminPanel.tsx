...
export default function AdminPanel(props: AdminPanelProps) {
  ...
  const nav: { id: View; label: string; count?: number }[] = [{ id: 'overview', label: 'Attention' }, { id: 'kyc', label: 'KYC', count: pendingKyc.length }, { id: 'users', label: 'Users' }, { id: 'listings', label: 'Listings', count: pendingListings.length }, { id: 'matches', label: 'Matches' }, { id: 'disputes', label: 'Disputes', count: openDisputes.length }, { id: 'analytics', label: 'Analytics' }, { id: 'audit', label: 'Audit Log' }];

  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs dark:border-slate-800 dark:bg-slate-900" id="admin_dashboard">
    <header className="border-b border-slate-200 px-5 py-4 dark:border-slate-800"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-base font-bold text-slate-950 dark:text-white">Admin dashboard</h2><p className="text-xs text-slate-500">Work queues and platform controls.</p></div><ShieldCheck className="h-5 w-5 text-emerald-600" aria-label="Admin access" /></div><div className="mt-4 grid grid-cols-2 divide-x divide-y border border-slate-200 text-xs sm:grid-cols-4 dark:divide-slate-800 dark:border-slate-800"><Metric onClick={() => choose('kyc')} label="KYC awaiting action" value={pendingKyc.length}/><Metric onClick={() => choose('listings')} label="Listings awaiting review" value={pendingListings.length}/><Metric onClick={() => choose('disputes')} label="Open disputes" value={openDisputes.length}/><Metric onClick={() => choose('analytics')} label="Active collaborations" value={activeCollaborations}/></div></header>
    <div className="flex flex-col lg:flex-row"><nav className="flex shrink-0 gap-1 overflow-x-auto border-b border-slate-200 p-2 lg:w-44 lg:flex-col lg:border-b-0 lg:border-r dark:border-slate-800" aria-label="Admin sections">{nav.map(item => <button key={item.id} onClick={() => choose(item.id)} className={`flex items-center justify-between rounded-lg px-3 py-2 text-left text-xs font-semibold ${view === item.id ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}>{item.label}{item.count ? <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] text-amber-900">{item.count}</span> : null}</button>)}</nav>
      <main className="min-w-0 flex-1 p-5">{view !== 'overview' && view !== 'analytics' && view !== 'audit' && <div className="mb-4 flex flex-wrap gap-2"><label className="relative min-w-52 flex-1"><Search className="absolute left-2 top-2 h-4 w-4 text-slate-400"/><input aria-label="Search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search" className="w-full rounded-lg border border-slate-200 py-2 pl-8 pr-3 text-xs dark:border-slate-700 dark:bg-slate-800" /></label>{view === 'kyc' && <Select value={kycStatus} setValue={setKycStatus} values={['ALL', 'PENDING', 'MORE_INFO', 'APPROVED', 'REJECTED']}/>} {view === 'listings' && <Select value={listingStatus} setValue={setListingStatus} values={['ALL', 'PENDING', 'APPROVED', 'REJECTED', 'SUSPENDED']}/>} {view === 'users' && <Select value={userRole} setValue={setUserRole} values={['ALL', UserRole.FARMER, UserRole.INVESTOR, UserRole.VETERINARIAN]}/>}</div>}
        {view === 'overview' && <AttentionRows pendingKyc={pendingKyc} pendingListings={pendingListings} openDisputes={openDisputes} onChoose={choose}/>} {view === 'kyc' && <QueueTable empty="No KYC records match this filter." headers={['Applicant', 'Document', 'Submitted', 'Status', '']} rows={filteredKyc.map(item => [<div key="applicant"><strong>{item.userName}</strong><span className="block text-[10px] text-slate-500">{item.userRole}</span></div>, `${item.documentType.replaceAll('_', ' ')} · ${item.documentNumber}`, new Date(item.submittedAt).toLocaleDateString(), <Status key="status" value={item.status}/>, <button key="review" onClick={() => { setSelectedKyc(item); setNote(item.adminNote || ''); }} className="link-action">Review</button>] )}/>} {view === 'users' && <QueueTable empty="No users match this filter." headers={['User', 'Role', 'County', 'Status', '']} rows={filteredUsers.map(user => [<div key="user"><strong>{user.name}</strong><span className="block text-[10px] text-slate-500">{user.email || user.phone}</span></div>, user.role, user.county, <Status key="status" value={user.verified ? 'APPROVED' : 'PENDING'}/>, user.role !== UserRole.ADMIN ? <button key="action" disabled={working} onClick={() => run(() => props.onApproveUser(user.id, user.verified ? 'REJECTED' : 'APPROVED'))} className="link-action">{user.verified ? 'Mark unverified' : 'Verify'}</button> : null])}/>} {view === 'listings' && <QueueTable empty="No listings match this filter." headers={['Listing', 'Owner', 'County', 'Status', '']} rows={filteredListings.map(item => [<div key="listing"><strong>{item.title}</strong><span className="block text-[10px] text-slate-500">KES {item.priceKES.toLocaleString()}</span></div>, item.ownerName, item.locationCounty, <Status key="status" value={item.moderationStatus || (item.verified ? 'APPROVED' : 'PENDING')}/>, <button key="review" onClick={() => { setSelectedListing(item); setNote(item.moderationNote || ''); }} className="link-action">Review</button>] )}/>} {view === 'matches' && <MatchWorkspace users={props.usersList} matches={props.matches} match={match} setMatch={setMatch} submit={createMatch} working={working}/>} {view === 'disputes' && <DisputeWorkspace disputes={props.disputes} leases={props.activeLeases} users={props.usersList} selected={selectedDispute} select={item => { setSelectedDispute(item); setNote(item.resolutionNotes || ''); }}/>} {view === 'analytics' && <Analytics analytics={props.analytics} collaborations={activeCollaborations}/>} {view === 'audit' && <AuditWorkspace />}</main></div>
    {selectedKyc && <ActionDialog title={`Review KYC: ${selectedKyc.userName}`} close={() => setSelectedKyc(null)} note={note} setNote={setNote} busy={working}><p className="text-xs text-slate-600 dark:text-slate-300">{selectedKyc.documentType.replaceAll('_', ' ')} · {selectedKyc.documentNumber}. Sensitive document details are only available through the protected record API.</p><History entries={selectedKyc.history || []}/><div className="flex flex-wrap gap-2"><button disabled={working} onClick={() => reviewKyc('APPROVED')} className="action-primary"><Check className="h-3.5 w-3.5"/>Approve</button><button disabled={working} onClick={() => reviewKyc('MORE_INFO')} className="action-secondary"><FileSearch className="h-3.5 w-3.5"/>Request information</button><button disabled={working} onClick={() => reviewKyc('REJECTED')} className="action-danger"><X className="h-3.5 w-3.5"/>Reject</button></div></ActionDialog>} {selectedListing && <ActionDialog title={`Moderate: ${selectedListing.title}`} close={() => setSelectedListing(null)} note={note} setNote={setNote} busy={working}><div className="flex flex-wrap gap-2"><button disabled={working} onClick={() => moderate('APPROVED')} className="action-primary"><Check className="h-3.5 w-3.5"/>Approve</button><button disabled={working} onClick={() => moderate('SUSPENDED')} className="action-secondary">Suspend</button><button disabled={working} onClick={() => moderate('REJECTED')} className="action-danger"><X className="h-3.5 w-3.5"/>Reject</button></div></ActionDialog>} {selectedDispute && <ActionDialog title={`Resolve dispute ${selectedDispute.id}`} close={() => setSelectedDispute(null)} note={note} setNote={setNote} busy={working}><select value={resolution} onChange={event => setResolution(event.target.value as typeof resolution)} className="w-full rounded-lg border border-slate-200 p-2 text-xs dark:border-slate-700 dark:bg-slate-800"><option value="refund_farmer">Refund escrow to farmer</option><option value="disburse_landowner">Release escrow to landowner</option></select><History entries={selectedDispute.history || []}/><button disabled={working || !note.trim()} onClick={resolve} className="action-primary"><Scale className="h-3.5 w-3.5"/>Record resolution</button></ActionDialog>}
  </section>;
}

function Metric({ label, value, onClick }: { label: string; value: number; onClick: () => void }) { return <button onClick={onClick} className="p-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800"><span className="block text-slate-500">{label}</span><strong className="text-lg text-slate-950 dark:text-white">{value}</strong></button>; }
function Select({ value, setValue, values }: { value: string; setValue: (value: string) => void; values: string[] }) { return <select value={value} onChange={event => setValue(event.target.value)} className="rounded-lg border border-slate-200 px-2 text-xs dark:border-slate-700 dark:bg-slate-800">{values.map(item => <option key={item} value={item}>{item === 'ALL' ? 'All statuses' : item.replaceAll('_', ' ')}</option>)}</select>; }
function AttentionRows({ pendingKyc, pendingListings, openDisputes, onChoose }: { pendingKyc: VerificationRequest[]; pendingListings: Listing[]; openDisputes: Dispute[]; onChoose: (view: View) => void }) { const rows = [{ label: 'KYC reviews', description: 'Verification submissions need a decision.', count: pendingKyc.length, view: 'kyc' as View }, { label: 'Listing reviews', description: 'New listings are not public until approved.', count: pendingListings.length, view: 'listings' as View }, { label: 'Dispute resolutions', description: 'Escrow-linked cases require a documented outcome.', count: openDisputes.length, view: 'disputes' as View }]; return <div><h3 className="text-sm font-bold text-slate-950 dark:text-white">Requires attention</h3><div className="mt-3 divide-y divide-slate-200 border-y border-slate-200 dark:divide-slate-800 dark:border-slate-800">{rows.map(row => <button key={row.view} onClick={() => onChoose(row.view)} className="flex w-full items-center gap-3 px-1 py-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800"><AlertTriangle className={`h-4 w-4 ${row.count ? 'text-amber-500' : 'text-slate-300'}`}/><div className="flex-1"><strong className="text-xs text-slate-900 dark:text-white">{row.label}</strong><span className="block text-[11px] text-slate-500">{row.description}</span></div><strong className="text-sm text-slate-900 dark:text-white">{row.count}</strong><ChevronRight className="h-4 w-4 text-slate-400"/></button>)}</div></div>; }
function QueueTable({ headers, rows, empty }: { headers: string[]; rows: React.ReactNode[][]; empty: string }) { return rows.length ? <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="border-b border-slate-200 text-[10px] uppercase tracking-wide text-slate-500 dark:border-slate-800"><tr>{headers.map(header => <th key={header} className="px-2 py-2 font-semibold">{header}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{rows.map((row, index) => <tr key={index}>{row.map((cell, cellIndex) => <td key={cellIndex} className="px-2 py-3 align-top text-slate-700 dark:text-slate-300">{cell}</td>)}</tr>)}</tbody></table></div> : <p className="py-10 text-center text-xs text-slate-500">{empty}</p>; }
